...
/update/${row.slug}`}
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${serverurl}delete-eventscategory/`, {
        method: 'POST',
        headers: {
  ...
  const getJobs = async (page) => {
    try {
      ...
      const url = user.role === '3' || user.role === 3
        ? `${serverurl}get-userjobs/?page=${page}`
        : `${serverurl}get-alleventcategory/?page=${page}`;
  ...
  const selectStatus = async (e) => {
    try {
      ...
      if (updateArray.length > 0) {
        ...
        const response = await fetch(`${serverurl}update-eventscategory/`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const breadcrumbItems = [
    ...
    { label: "All event Categories", href: "/events/category" },
