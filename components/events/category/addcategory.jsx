...
const AddCategory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      form.append("languages",selectedLanguages[0].value);
      form.append("approved", user_meta.role === 1 ? "1" : "0");

     if (typeof selectedIcon === "string") {
  form.append("icon", selectedIcon); // Icon name for predefined icons
} else {
  form.append("icon", selectedIcon); // Uploaded SVG file
}
  


      const response = await fetch(`${serverurl}add-eventcategory/`, {
        method: "POST",
        body: form,
      });
      ...
      toast.error("An unexpected error occurred,Category already exist ", {
        position: "top-right",
      });
  ...
  const breadcrumbItems = [
    { label: "Home", href: "/" },
    { label: "Add event category", href: "/events/category/add" },
  ];
  return (
    <div className="p-2">
