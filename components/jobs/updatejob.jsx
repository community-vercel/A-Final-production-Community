...
const UpdateJob= () => {
    ...
    const getData = async () => {
      try {
        ...
        setSlug(result.data.slug);
          setCurrentLogo(result.data.logo); // Assuming `logo` is the field name
          ...
          setKeywords(result.data.keyword.split(","))
            ...
            setExperienceLevel(result.data.experience_level)
            setSkill(result.data.skills_required.split(","))
            setApplicationDeadline(result.data.application_deadline)
            setContactInfo(result.data.contact_information)
            setSelectedLanguages({
              ...
              GetState(countryid).then((states) => {
                states.map((state) => {
                  if (state.name === result.data.state) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      form.append("location", place?.name?place.name:location);
      form.append("lat",lati?lati:lat);
      form.append("longitude",lngi?lngi:longitude);

      form.append("state", selectedState || selectedStateDB.name);
      form.append("city", selectedCity || selectedCityDB.name);
      form.append("zip", zip);
      ...
      const response = await fetch(`${serverurl}update-jobdata/`, {
        method: "POST",
        body: form,
      });
  ...
  const breadcrumbItems = [
    { label: "Home", href: "/" },
    { label: `Update Job (${params.slug.toUpperCase()})`, href: `/jobs/update/${params.slug}` },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="col-span-2">
            ...
            <Autocomplete
                 ...
                 <input
              type="text"
            value={place?.name?place.name:location}
          ...
          {currentLogo && (
            <img src={serverurl+currentLogo.replace('/api/media/', 'media/')} alt="Current Logo" className="mt-2 mb-2 h-20" />
          )}
