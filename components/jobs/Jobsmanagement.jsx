...
{(user.role === 1 || user.role === '1' || user.id === row.user_id) && (
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${serverurl}delete-jobs/`, {
        method: 'POST',
        headers: {
  ...
  const getJobs = async (page) => {
    try {
      ...
      const url = user.role === '3' || user.role === 3
        ...
        : `${serverurl}get-alljobs/?page=${page}`;
  ...
  const selectStatus = async (e) => {
    try {
      ...
      if (updateArray.length > 0) {
        ...
        const response = await fetch(`${serverurl}update-jobs/`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
