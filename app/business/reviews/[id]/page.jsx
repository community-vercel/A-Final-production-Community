...
body:JSON.stringify(formdata)
  ...
  const onSubmit = async (formData) => {
    try {
      const existingImages = typeof imagesDB === 'string' ? imagesDB.split(",") : [];
...
<Breadcrumb items={breadcrumbItems} /> 
      ...
      {loading ? (
        ...
        <>
                  <div>
    <H2 className="flex items-center justify-center ">Update Review</H2>
</div>
        <div className="max-w-4xl bg-white my-3 mx-auto">
  ...
  {[
  ...(typeof imagesDB === "string" ? imagesDB.split(",") : []),
  ...images,
].map((item, index) => {
  // Clean image path if it's a string and contains '/api/'
  const cleanItem =
    typeof item === "string" && item.includes("/api/")
      ? item.replace("/api/", "")
      : item;
      ...
      <div className="relative h-full" key={index}>
        <Image
          src={
            typeof cleanItem === "string"
              ? serverurl + cleanItem // For existing images
        ...
        <XMarkIcon
          ...
          onClick={() =>
            imgDelete(
              typeof cleanItem === "string"
                ? item // Use original item for string paths
                : cleanItem.name // Use name for file objects
            )
          }
        />
      </div>
